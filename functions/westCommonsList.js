...
export default async function getWestCommonsList(date) {
    const Now = date ? new Date(date) : new Date()
    const formattedDate = `${Now.getFullYear()}-${(Now.getMonth() + 1).toString().padStart(2, "0")}-${Now.getDate().toString().padStart(2, "0")}`
    let days;
    days = foodCache.get(formattedDate);
    if (days == undefined) {
        const food = await fetch(`https://woodward.nutrislice.com/menu/api/weeks/school/west-commons/menu-type/lunch/${Now.getFullYear()}/${Now.getMonth() + 1}/${Now.getDate()}`)
